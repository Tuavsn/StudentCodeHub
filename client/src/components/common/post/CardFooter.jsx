...
const CardFooter = ({ post, type }) => {
    const { auth, socket, posts } = useSelector((state) => state)
    ...
    const handleSubmit = (e) => {
        ...
        if(Object.keys(errors).length === 0) {
            ...
            dispatch(createComment({post, newComment, auth, socket, type}))
        } else {
            setErrors(errors)
        }
    ...
    const handleLike = async () => {
        if(loadLike) return
        setLoadLike(true)
        await dispatch(likePost({post, auth, socket, type}))
        setLoadLike(false)
    }
    ...
    const handleUnLike = async () => {
        if(loadLike) return
        setLoadLike(true)
        await dispatch(unLikePost({post, auth, socket, type}))
        setLoadLike(false)
    }
